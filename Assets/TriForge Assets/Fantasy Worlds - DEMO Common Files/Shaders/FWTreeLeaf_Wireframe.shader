...
Shader "TriForge/Fantasy Worlds/FWTreeLeaf_Wireframe"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			struct VertexInput
			{
				...
				float4 bakedBary : TEXCOORD6; // UV7
			...
			VertexOutput VertexFunction( VertexInput v  )
			{
				...
				// Pass Baked Barycentrics
				bool hasData = v.bakedBary.w > 0.5 && length(v.bakedBary.xyz) > 0.1;
				o.barycentric = hasData ? v.bakedBary.xyz : float3(1,1,1);
				return o;
			}
						...
						, bool ase_vface : SV_IsFrontFace ) : SV_Target
			{
				...
				// Alpha Clip
				clip(color.a - _MaskClip);

				// Wireframe Mix
				float3 barys = IN.barycentric;
				float3 deltas = fwidth(barys);
				float3 smoothing = deltas * _WireSmoothing;
				float3 thickness = deltas * (_WireThickness / 20.0);
				float minBary = min(barys.x, min(barys.y, barys.z));
				float edgeIntensity = smoothstep(thickness.x, thickness.x + smoothing.x, minBary);
