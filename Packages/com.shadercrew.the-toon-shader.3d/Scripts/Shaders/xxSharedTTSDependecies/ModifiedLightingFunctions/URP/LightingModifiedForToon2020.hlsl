#ifndef UNIVERSAL_LIGHTING_MODIFIED_FOR_TOON
#define UNIVERSAL_LIGHTING_MODIFIED_FOR_TOON
...
half3 LightingLambert(half3 lightColor, half modifiedLightIntensity)
{
    ...
    half smoothness, half occlusion, half3 emission, half alpha, ToonShadingData toonShadingData)
{
    ...
    return UniversalFragmentPBR(inputData, s, toonShadingData);
}



half4 UniversalFragmentBlinnPhong(InputData inputData, half3 diffuse, half4 specularGloss, half smoothness, half3 emission, half alpha, ToonShadingData toonShadingData)
{
    ...
    float modifiedLightIntensity = CalculateCellShadingPartitioning(mainLight.direction, toonShadingData);
    
    //half3 diffuseColor = inputData.bakedGI + LightingLambert(attenuatedLightColor, mainLight.direction, inputData.normalWS, modifiedLightIntensity);
    half3 diffuseColor = inputData.bakedGI + LightingLambert(attenuatedLightColor, modifiedLightIntensity);

    
    
    half3 specularColor = LightingSpecular(attenuatedLightColor, mainLight.direction, inputData.normalWS, inputData.viewDirectionWS, specularGloss, smoothness, toonShadingData);
    ...
    for (uint lightIndex = 0u; lightIndex < pixelLightCount; ++lightIndex)
    {
        ...
        float modifiedLightIntensityAdditionalLight = CalculateCellShadingPartitioning(light.direction, toonShadingData);
    
        half3 attenuatedLightColor = light.color * (light.distanceAttenuation * light.shadowAttenuation);
        diffuseColor += LightingLambert(attenuatedLightColor, modifiedLightIntensityAdditionalLight);
        //diffuseColor += LightingLambert(attenuatedLightColor, light.direction, inputData.normalWS);
        specularColor += LightingSpecular(attenuatedLightColor, light.direction, inputData.normalWS, inputData.viewDirectionWS, specularGloss, smoothness,toonShadingData);
    }
#endif
...
//half4 LightweightFragmentBlinnPhong(InputData inputData, half3 diffuse, half4 specularGloss, half smoothness, half3 emission, half alpha, half modifiedLightIntensity)
//{
//    return UniversalFragmentBlinnPhong(inputData, diffuse, specularGloss, smoothness, emission, alpha, modifiedLightIntensity);
//}
